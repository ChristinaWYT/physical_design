...
number_bytes_read=[]
workloadb_start_timestamp=[]
workloadb_end_timestamp = []
...
for case in cases:
	with open('case'+case+'.log', 'r') as file:
		...
		workloadb_start_timestamp.append(int(m[6]))
		workloadb_end_timestamp.append(int(m[7]))
...
def metricstats(file_name, master, metric):
	...
	for case in cases:
		...
		with open(filename, 'r') as file:
			...
			f = file.read()
			count = 0
			stamp = workloadb_end_timestamp[casecount]
			...
			found = False
			count = 0
			stamp = workloadb_start_timestamp[casecount]
			while not found and count < 1000:
				regex = "("+str(stamp)+".*)"
				matches = re.findall(regex, f)
				print(matches)
...
def compactionstats(file_name, master, metric):
	...
	for case in cases:
		...
		with open(filename, 'r') as file:
			found = False
			count=0
			end_stamp = workloadb_end_timestamp[casecount]
			start_stamp = workloadb_start_timestamp[casecount]
			for line in file:
				words=line.split()
...
pyplot.savefig('workloadb_' + args.cases + '.png')
pyplot.show()
