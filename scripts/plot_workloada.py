...
number_bytes_read=[]
workloada_start_timestamp=[]
workloada_end_timestamp = []
...
for case in cases:
	with open('case'+case+'.log', 'r') as file:
		...
		workloada_start_timestamp.append(int(m[4]))
		workloada_end_timestamp.append(int(m[5]))
...
def metricstats(file_name, master, metric):
	...
	for case in cases:
		...
		with open(filename, 'r') as file:
			...
			f = file.read()
			count = 0
			stamp = workloada_end_timestamp[casecount]
			...
			found = False
			count = 0
			stamp = workloada_start_timestamp[casecount]
			while not found and count < 1000:
				regex = "("+str(stamp)+".*)"
				matches = re.findall(regex, f)
				print(matches)
...
def compactionstats(file_name, master, metric):
	...
	for case in cases:
		...
		with open(filename, 'r') as file:
			found = False
			count=0
			end_stamp = workloada_end_timestamp[casecount]
			start_stamp = workloada_start_timestamp[casecount]
			for line in file:
				words=line.split()
...
pyplot.savefig('workloada_' + args.cases + '.png')
pyplot.show()
