...
for case in cases:
	with open('case'+case+'.log', 'r') as file:
		...
		read_start_timestamp.append(int(m[2]))
		read_end_timestamp.append(int(m[3]))
...
def metricstats(file_name, master, metric):
	...
	for case in cases:
		...
		with open(filename, 'r') as file:
			...
			f = file.read()
			count = 0
			stamp = read_end_timestamp[casecount]
			...
			found = False
			count = 0
			stamp = read_start_timestamp[casecount]
			while not found and count < 1000:
				regex = "("+str(stamp)+".*)"
				matches = re.findall(regex, f)
				print(matches)
...
def compactionstats(file_name, master, metric):
	...
	for case in cases:
		...
		with open(filename, 'r') as file:
			found = False
			count=0
			end_stamp = read_end_timestamp[casecount]
			start_stamp = read_start_timestamp[casecount]
			for line in file:
				words=line.split()
...
p1, = ax1.plot([1,2,3], load_throughputs, color='r', marker='o', label="random read throughput", linewidth=1.5)
ax2 = ax1.twinx()
...
ax4 = ax1.twinx()
p4, = ax4.plot([1,2,3], number_storeFiles, color='k', marker='o', label="change in number of storeFiles", linewidth=1.5)
ax5 = ax1.twinx()
...
pyplot.savefig('random_read_' + args.cases + '.png')
pyplot.show()
