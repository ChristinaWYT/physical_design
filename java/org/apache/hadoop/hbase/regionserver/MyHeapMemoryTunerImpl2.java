...
class MyHeapMemoryTunerImpl2 implements HeapMemoryTuner {
  ...
  private static final float tuning_period = 180.0f;
  private static final int avg_throughput = 15000;
  ...
  private static float read_prop=0.5f;
  private static float write_prop=0.5f;
  ...
  public static void setReadWriteProp() throws Exception {
    	String line = readpropfile.readLine();
	read_prop = Float.valueOf(line).floatValue();
	write_prop = 1.0f - read_prop;
	System.out.println("read proportion = " + read_prop);
        System.out.println("write_proportion = " + write_prop);
  }
  ...
  public TunerResult tune(TunerContext context) {
    ...
    try {
      ...
      readpropfile = new RandomAccessFile("/home/hadoop/hbase/readpropfile", "r");
    ...
    System.out.println("selfdapql blockedFlushCount: " + blockedFlushCount);
    System.out.println("selfdapql unblockedFlushCount: " + unblockedFlushCount);
    System.out.println("selfdapql evictCount: " + evictCount);
    boolean memstoreSufficient = rslog_blocking_updates_count == 0;
    boolean blockCacheSufficient = am_blockCacheEvictionCount == 0;
    if (memstoreSufficient && blockCacheSufficient) {
      System.out.println("selfdapql No Tuning");
      return NO_OP_TUNER_RESULT;
    }
    boolean succifient_cache = false;
    float newMemstoreSize=context.getCurMemStoreSize();
    float newBlockCacheSize=context.getCurBlockCacheSize();
    float currentCacheSize = context.getCurBlockCacheSize()*12.0f;
    System.out.println("selfdapql current cache size: " + currentCacheSize);
    System.out.println("selfdapql data size: " + am_storeFileSize);
    float cachedataratio = currentCacheSize/(4.0f * am_storeFileSize);
    System.out.println("selfdapql cache-data ratio: " + cachedataratio);
    if(cachedataratio>1) {
	succifient_cache = true;
        System.out.println("enough cache");
        if(rslog_blocking_updates_count > 0) {
          System.out.println("rslog_blocking_updates_count: " + rslog_blocking_updates_count);
          newBlockCacheSize = context.getCurBlockCacheSize() - step;
          newMemstoreSize = context.getCurMemStoreSize() + step;
        }
    }
    else {
      //float ratio = ((float) 64.0f/(currentCacheSize*1024))*(1/(1-cachedataratio));
      //float ratio = CONSTANT*(1/(1-cachedataratio));
      //System.out.println("selfdapql probability ratio: " + ratio);
      float deltacache_to_data_ratio = (DEFAULT_STEP_VALUE * 12.0f) / (4.0f* am_storeFileSize);
      System.out.println("deltacache_to_data_ratio: " + deltacache_to_data_ratio); 
      float pc_by_pm = ( read_prop *dnmetrics_avg_read_time)/(write_prop * blocking_period_per_update);     
      System.out.println("selfdapql pc by pm ratio: " + pc_by_pm);
      float cache_cost_per_second = deltacache_to_data_ratio * read_prop * avg_throughput; 
      System.out.println("selfdapql pc_by_pm * cache_cost_per_second: " + pc_by_pm * cache_cost_per_second);
      System.out.println("selfdapql rslog_blocking_updates_count per second: " + ((float)rslog_blocking_updates_count/tuning_period)); 
      float normalized_blockCachecount = pc_by_pm * cache_cost_per_second;
      System.out.println("selfdapql normalized_blockCachecount: " + normalized_blockCachecount);
      float normalized_count_diff = normalized_blockCachecount - ((float)rslog_blocking_updates_count/tuning_period);
      if(normalized_count_diff > 0) {
         newBlockCacheSize = context.getCurBlockCacheSize() + step;
         newMemstoreSize = context.getCurMemStoreSize() - step;
      }
      else {
