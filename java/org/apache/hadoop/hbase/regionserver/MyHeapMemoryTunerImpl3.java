...
class MyHeapMemoryTunerImpl3 implements HeapMemoryTuner {
  ...
  private static RandomAccessFile thresholdfile = null;
  ...
  public float getMemstoreFlushThreshold() throws Exception {
	String line = thresholdfile.readLine();
        float threshold = Float.valueOf(line).floatValue();
        System.out.println("memstore flush threshold: " + threshold);
        return(threshold);
  }
  ...
  public TunerResult tune(TunerContext context) {
    ...
    try {
      ...
      thresholdfile =  new RandomAccessFile("/home/hadoop/hbase/thresholdfile", "r");
      
      MEMSTORE_FLUSH_THRESHOLD = getMemstoreFlushThreshold();
