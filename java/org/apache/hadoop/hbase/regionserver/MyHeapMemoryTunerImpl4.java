...
class MyHeapMemoryTunerImpl4 implements HeapMemoryTuner {
  ...
  public static float MEMSTORE_FLUSH_THRESHOLD=30.0f;
  public static float EVICTION_THRESHOLD=50000.0f;
  ...
  private static RandomAccessFile memstore_flush_thresholdfile = null;
  private static RandomAccessFile eviction_thresholdfile = null;
  ...
  public float getThreshold(RandomAccessFile thresholdfile) throws Exception {
	String line = thresholdfile.readLine();
        float threshold = Float.valueOf(line).floatValue();
  ...
  public TunerResult tune(TunerContext context) {
    ...
    try {
      ...
      memstore_flush_thresholdfile =  new RandomAccessFile("/home/hadoop/hbase/mem_thresholdfile", "r");
      eviction_thresholdfile = new RandomAccessFile("/home/hadoop/hbase/evict_thresholdfile","r");
      MEMSTORE_FLUSH_THRESHOLD = getThreshold(memstore_flush_thresholdfile);
      EVICTION_THRESHOLD = getThreshold(eviction_thresholdfile);
    ...
    if (memstoreSufficient) {
      ...
      newBlockCacheSize = context.getCurBlockCacheSize() - step;
      newMemstoreSize = context.getCurMemStoreSize() + step;
    } else if(blockedFlushCount> MEMSTORE_FLUSH_THRESHOLD) {
	newBlockCacheSize = context.getCurBlockCacheSize() - step;
      	newMemstoreSize = context.getCurMemStoreSize() + step;
    } else if(evictCount > EVICTION_THRESHOLD) {
      newBlockCacheSize = context.getCurBlockCacheSize() + step;
      newMemstoreSize = context.getCurMemStoreSize() - step;
    } else {
      return NO_OP_TUNER_RESULT;
